...
let createRequest
  ...
  if seriesName.IsSome then
    req.SeriesName <- seriesName.Value

  if seasonNum.IsSome then
    req.ParentIndexNumber <- Nullable(seasonNum.Value)

  if episodeNum.IsSome then
    req.IndexNumber <- Nullable(episodeNum.Value)

  if year.IsSome then
    req.ProductionYear <- Nullable(year.Value)

  req.ProviderIds <- Dictionary(StringComparer.OrdinalIgnoreCase)

  if imdbId.IsSome then
    req.ProviderIds.["Imdb"] <- imdbId.Value

  if tmdbId.IsSome then
    req.ProviderIds.["Tmdb"] <- tmdbId.Value
...
let test_extractMetadata_movie_with_imdb () =
  let request =
    createRequest "The Matrix" None None None (Some 1999) (Some "tt0133093") None
...
let test_extractMetadata_episode () =
  let request =
    createRequest "Breaking Bad" (Some "Breaking Bad") (Some 1) (Some 1) (Some 2008) None None
...
let test_isHighConfidenceMatch () =
  ...
  let request1 =
    createRequest "Inception" None None None (Some 2010) (Some "tt1375666") None
...
let test_hasReliableMetadata () =
  ...
  let request2 =
    createRequest "ShowName" (Some "Breaking Bad") (Some 5) (Some 14) None None None
  ...
  let request3 =
    createRequest "ShowName" (Some "Breaking Bad") None None None None None
...
let test_metadata_extraction_complete () =
  let request =
    createRequest "The Dark Knight Rises" None None None (Some 2012) (Some "tt1345836") (Some "49026")
